...
import { Button as ButtonNative, Overlay } from "react-native-elements";
...
export default class CheckoutScreen extends Component {
  ...
  async componentDidMount() {
    ...
    if (
      typeof hotel_arr != "undefined" &&
      hotel_arr != null &&
      hotel_arr.length != null &&
      hotel_arr.length > 0
    ) {
      this.setState({
        hotel_exist: true,
  ...
  async CheckOutButtonHandleClick() {
    ...
    console.log(
      " (CheckOutButtonHandleClick:51 )" + this.state.guests,
      this.state.check_in_date,
      this.state.check_out_date
    );

    // const first = await paypal.getPaymentData("PAYID-MHT6S6Q62U7110935628291L");
    // const transaction_data = first.data;
    // console.log(transaction_data.total_paid);

    const promise = paypal.checkOutHandle(
      this.state.check_in_date,
      this.state.check_out_date,
      this.state.guests,
      "1"
    ...
    if (
      typeof this.state.paypal_html_data.data !== "undefined" &&
      this.state.paypal_html_data
    ) {
      paymentToken =
        this.state.paypal_html_data.data.contextualLogin.paymentToken;
  ...
  async _onNavigationStateChange(webViewState) {
    if (
      webViewState.url.includes("http://tourism.nhely.hu/api/api-status") &&
      ...
      const paypal = new PayPal();
      const promise = await paypal.getURLData(webViewState.url);
      const paymentID = promise.data.paymentId;
      const payerID = promise.data.PayerID;
      console.log(paymentID, payerID);
      const payment_promise = await paypal.getPaymentData(paymentID);
      const transaction_data = payment_promise.data;
      console.log(transaction_data.total_paid);

      this.setState({
        pp_transaction_data: transaction_data,
        isModalOpen: true,
      });
    }
  }
        ...
        <Box style={{  height: "100%" }}>
            ...
            <HotelExist hotelExist={this.state.hotel_exist}>
              <Box m="8" >
                <CheckoutModal 
                  ...
                  transactionData={this.state.pp_transaction_data}
                />
                <CheckIn
                  onChange={(event, selectedDate) =>
                    this.CheckInChanged(event, selectedDate)
                  }
                  shown={this.state.show_date_picker_check_in}
                  currentvalue={this.state.check_in_date}
                />
                <CheckOut
                  onChange={(event, selectedDate) =>
                    this.CheckOutChanged(event, selectedDate)
                  }
                  shown={this.state.show_date_picker_check_out}
                  currentvalue={this.state.check_out_date}
                />
                ...
                <Button
                  ...
                  m="2"
                  colorScheme="emerald"
                  onPress={() => this.handleCheckInButtonState()}
                ...
                <Button
                  ...
                  m="2"
                  colorScheme="emerald"
                  onPress={() => this.handleCheckOutButtonState()}
                  leftIcon={<Icon as={Ionicons} name="calendar-sharp" size="sm" />}
                >
                  Select Check Out Date!
                </Button>
                <Text color="coolGray.600" fontSize="md">Select the number of guests:</Text>
                <Input
                ...
                />
                <Errors
                  errorData={this.state.paypal_html_data}
                  shown={this.state.error_happened}
                ></Errors>
              </Box>
            </HotelExist>
          </ContentDynamic>
          ...
          <HotelNotExist hotelExist={this.state.hotel_exist}>
            <ContentDynamic shown={this.state.content_before_checkout_show}>
              <Center>
                <Image
                  source={require("../assets/nohotel.png")}
                  alt="image"
                  style={{ resizeMode: "contain", width: "90%" }}
                />
                <Text fontSize="xl" color="coolGray.600">
                  No available hotel found, right now!
                </Text>
                <ButtonNative
                  title="GO BACK!"
                  onPress={() => {
